...
const temporary = await mkdtemp(join(tmpdir(), "pontx-notion-pack-"));
...
try {
  ...
  await execFileAsync(process.execPath, ["--input-type=module", "--eval", [
    ...
    "if (typeof sdk.createNotionClient !== 'function') process.exit(1);",
    "if (!sdk.default || !sdk.default.users) process.exit(2);",
    "});",
  ].join("")], { cwd: temporary });
  const cli = join(temporary, "node_modules/.bin/pontx-notion");
